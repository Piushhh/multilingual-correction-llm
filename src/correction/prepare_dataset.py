...
def load_data(input_path: Union[str, Path]) -> List[Dict[str, Any]]:
    data = []
...
def validate_and_clean(data: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    valid_data = []
    seen_ids = set()
    seen_texts = set()
    ...
    for item in data:
        ...
        if not str(item["incorrect_text"]).strip() or not str(item["correct_text"]).strip():
...
def create_splits(data: List[Dict[str, Any]], seed: int = 42, train_ratio: float = 0.8, val_ratio: float = 0.1) -> List[Dict[str, Any]]:
    random.seed(seed)
    random.shuffle(data)
...
def save_data(data: List[Dict[str, Any]], output_dir: Union[str, Path]) -> None:
